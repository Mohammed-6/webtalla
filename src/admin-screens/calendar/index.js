...
class Calender extends Component {
    ...
    create
      .post(
        process.env.REACT_APP_BASEURL +
          "admin/allCalender?access_token=" +
  ...
  handleValidEventSubmit = (e, values) => {
    ...
    if (isEdit) {
      ...
      formData.append("cal_data", JSON.stringify(updateEvent))
      create
        .post(
          process.env.REACT_APP_BASEURL +
            "admin/updateCalender?access_token=" +
            localStorage.getItem("adminToken"),
          formData,
          {
      ...
      const formData = new FormData()
      formData.append("cal_data", JSON.stringify(newEvent))
      create
        .post(
          process.env.REACT_APP_BASEURL +
            "admin/addCalender?access_token=" +
            localStorage.getItem("adminToken"),
          formData,
          {
            headers: { "Content-Type": "multipart/form-data" },
  ...
  handleDeleteEvent = () => {
    ...
    const formData = new FormData()
    formData.append("cal_data", JSON.stringify(event))
    create
      .post(
        process.env.REACT_APP_BASEURL +
          "admin/deleteCalender?access_token=" +
          localStorage.getItem("adminToken"),
        formData,
        {
          headers: { "Content-Type": "multipart/form-data" },
                    ...
                    <Row>
                      <Col lg={3}>
                        ...
                        <div id="external-events" className="mt-3">
                          ...
                          {this.state.caldata &&
                            this.state.caldata.map((category, i) => (
                              <div
                                key={i}
                                className={`${category.className} external-event text-white p-1 mb-2`}
                                draggable
